...
class AICDSummary():
    ...
    def plot_picks(self):
        ...
        for i in range(len(picks)):
            plt.plot([(picks[i] - self.tr.stats.starttime), (picks[i] - self.tr.stats.starttime)],
                     [min(self.tr), max(self.tr)], 'k--', label="Pick")
    ...
    def plot_summary(self):
        ...
        ax.plot(t, self.tr, c='gray', label="raw data")
        plt.ylabel('Raw Data')

        # Plot AIC function
        ax1 = plt.subplot(3, 1, 2)
        ax1.plot(t, self.aic, c='k', label="AIC function")
        plt.ylabel('AIC Function')

        # Plot summary
        ax2 = plt.subplot(3, 1, 3)
        ax2.plot(t, self.summary / np.amax(self.summary), c='k', label="Normalized CF")
        ax2.plot(t, self.thres / np.amax(self.summary), '--', linewidth=2.0, c='k', label="Threshold")
